from middleware.engine import MiddlewareEngine
...
if __name__ == '__main__':
    ...
    for placeholder, control_char in replacements.items():
        message = message.replace(placeholder, control_char)
    
    engine = MiddlewareEngine()
    config = engine.select_analyzer("Erba")
    
    msg_str = str(message)
    validated_message = engine.handler.process_data(msg_str)
